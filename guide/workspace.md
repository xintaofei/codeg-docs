...
- **Fork from here.** A finished reply offers a fork in its stats row: the new session inherits the conversation and the original is untouched, live or reopened alike. The action appears on a connected session whose agent **advertises** forking — that's read off the handshake rather than a fixed list, and today includes Claude Code, Codex (since **0.30.1**, opening with the full history it inherited) and DeepSeek Harness. Where it forks *from* is the part worth knowing: **Claude Code** and **Codex** can name an individual reply, so the fork lands exactly there; everywhere else — and on those two in the odd case where the turn carries nothing to name it by — it falls back to the **end of the conversation** rather than failing. A reply still streaming can't be forked at all, because naming a message the agent is still writing is not a thing that can mean anything.
...
- **Context compaction.** When an agent runs out of room and compacts its own context, a card says so while it happens and reports **tokens before → after** when the agent supplies them. A compaction that fails says *failed* rather than sitting at *in progress* forever.
