...
### 跟随最新发布，而不是那个固定版本 {#adapter-version}

上面 Kimi 的故事有两面：一个经过审核的固定版本，既是把坏版本挡在外面的东西，也是横在你和今早刚上线的新模型之间的东西。从 **0.30.3** 起，这笔账由你自己按智能体来算。**设置 → 智能体**里该智能体的[详情面板](/zh/guide/agents#configure-an-agent)带着一个 **适配器版本** 下拉框，有两个取值：

- **固定（推荐）** —— Codeg 审核过的那个版本。这是默认值，也是那些你不想操心的智能体该待的地方。
- **最新（未经审核）** —— 你安装的那一刻，这个包在 npm 上最新的发布。它正是让一个全新模型在上线当天就能用上的东西，而它确实未经审核：某个发布可能让这个智能体在 Codeg 里不可用，偶尔还有已知是坏的版本。

在动它之前，有三件事值得知道：

- **它只在安装与升级时被读取**，而且读的是**已保存**的那份设置。后台不会去轮询 npm，启动时跑的是磁盘上现有的那一份。所以光改下拉框本身什么也不会发生：先按**保存**，*再*按**升级**。不保存就升级，你装回去的还是那个固定版本。
- **取最新失败时会改试固定版本**，并在安装日志里留一句说明，于是 npm 不可达、或某个镜像还没同步到这个 tag，都会退化成那个经过审核的版本，而不是直接失败。如果这次重试也失败，安装就失败了 —— 而由于升级会先卸载再安装，这种情况可能让这个智能体处于未安装状态。
- **它只对 npx 类智能体有效。** 二进制或 uvx 安装没有 npm dist-tag 可以跟随，所以那里根本不会出现这个下拉框。
- **填了自定义版本就完全覆盖它**，两条通道上都一样，而且绝不回退：你要的就是那个确切的版本，悄悄装上另一个等于替你改了标签。

一旦通道与固定版本可能不一致，同一面板里的**版本状态**就是该信的那个号码 —— 它报告的是本地探测到的结果。对 npx 类智能体来说那次探测是 `npm list`，而它本身也可能因为别的原因失败；失败时会退回到上一次记录下来的版本，而不是宣称什么都没装。所以一个在升级失败之后仍然存在的读数，值得再确认一遍。
