...
多数时候你根本不用自己去打开它。**自动打开预览**——**设置 → 技能包 → 办公工具** 顶部那个开关，**默认开启**——会在智能体第一次创建或编辑 Word、Excel、PowerPoint 文件时自动开出实时预览，于是文档是一边被写一边出现在你眼前，而不是等你想起来去找。

它对一份文档只自动开**一次**。你关掉的预览就保持关闭：自 **0.31.2** 起，你亲手打开过的标签页算作「已经见过这个文件」，因此关掉它不会给下一次编辑一个重新弹出的理由——而**已被删除**的文档也不会再被打开，从前智能体自己的清理动作要是监视器最后报上来的那一条，就会出现这种情况。
